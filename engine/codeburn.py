...
def _scan_sessions(date_from: datetime, date_to: datetime) -> dict:
    ...
    for turn in turns:
        ...
        for api_call in turn["api_calls"]:
            usage = api_call.get("usage", {})
            if usage:
                daily_agg[turn["date"]]["input_tokens"] += usage.get("input_tokens", 0)
                daily_agg[turn["date"]]["output_tokens"] += usage.get("output_tokens", 0)
                daily_agg[turn["date"]]["cache_read_tokens"] += usage.get("cache_read_input_tokens", 0)
                daily_agg[turn["date"]]["cache_create_tokens"] += usage.get("cache_creation_input_tokens", 0)
    ...
    for d in daily:
        ...
        if wk not in weekly_agg:
            weekly_agg[wk] = {
                "week": wk, "tool_calls": 0,
                "input_tokens": 0, "output_tokens": 0,
                "cache_read_tokens": 0, "cache_create_tokens": 0,
                "cost_usd": 0.0, "turns": 0, "days": 0,
            }
            weekly_order.append(wk)
        w = weekly_agg[wk]
        w["tool_calls"] += d["tool_calls"]
        w["input_tokens"] += d["input_tokens"]
        w["output_tokens"] += d["output_tokens"]
        w["cache_read_tokens"] += d["cache_read_tokens"]
        w["cache_create_tokens"] += d["cache_create_tokens"]
        w["cost_usd"] += d["cost_usd"]
        w["turns"] += d["turns"]
        w["days"] += 1
    ...
    for wk in weekly_order:
        w = weekly_agg[wk]
        total_tk = (w["input_tokens"] + w["output_tokens"]
                    + w["cache_read_tokens"] + w["cache_create_tokens"])
        weekly.append({
            "week": wk,
            ...
            "cache_create_tokens": w["cache_create_tokens"],
            "total_tokens": total_tk,
            "tokens_per_call": round(total_tk / w["tool_calls"]) if w["tool_calls"] else 0,
    ...
    # Efficiency summary
    total_tool_calls = sum(d["tool_calls"] for d in daily)
    ...
    efficiency = {
        ...
        "mean_tokens_per_call": (
            round(total_all_tokens / total_tool_calls)
            if total_tool_calls else 0
        ),
        "weekly": weekly,
    }
